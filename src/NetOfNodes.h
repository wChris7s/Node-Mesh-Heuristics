...
void NetOfNodes::buildMesh(int obstacles) {
    buildConnections();
    deleteRandomNodes();
...
std::pair<int, int> NetOfNodes::setInitEnd() {
    ...
    while (!sePuedeCrear) {
        ...
        if (net[startX][startY].getX() != -1 && net[startX][startY].getY() != -1 &&
            !net[startX][startY].getObstacle() && !net[startX][startY].getDeleted()) {
...
void NetOfNodes::bestFirst() {
    ...
    while (!openList.empty()) {
        ...
        for (Node *neighbor: neighbors) {
            if (!neighbor->getVisited() && std::find(closedList.begin(), closedList.end(), neighbor) == closedList.end()) {
                // Verifica que los nodos de la lista abierta no esten en la lista cerrada.
...
std::vector<Node *> NetOfNodes::getNeighbors(Node *node) {
    std::vector<Node*> neighbors;
    int x = node->getX();
    int y = node->getY();

    if (x > 0 && !net[x - 1][y].getVisited() && !net[x - 1][y].getDeleted() && !net[x - 1][y].getObstacle()) {
        neighbors.push_back(&net[x - 1][y]);
    }
    if (x < getROWS() - 1 && !net[x + 1][y].getVisited() && !net[x + 1][y].getDeleted() &&
        !net[x + 1][y].getObstacle()) {
        neighbors.push_back(&net[x + 1][y]);
    }
    if (y > 0 && !net[x][y - 1].getVisited() && !net[x][y - 1].getDeleted() && !net[x][y - 1].getObstacle()) {
        neighbors.push_back(&net[x][y - 1]);
    }
    if (y < getCOLS() - 1 && !net[x][y + 1].getVisited() && !net[x][y + 1].getDeleted() &&
        !net[x][y + 1].getObstacle()) {
        neighbors.push_back(&net[x][y + 1]);
    }
    if (x > 0 && y > 0 && !net[x - 1][y - 1].getVisited() && !net[x - 1][y - 1].getDeleted() &&
        !net[x - 1][y - 1].getObstacle()) {
        neighbors.push_back(&net[x - 1][y - 1]);
    }
    if (x > 0 && y < getCOLS() - 1 && !net[x - 1][y + 1].getVisited() && !net[x - 1][y + 1].getDeleted() &&
        !net[x - 1][y + 1].getObstacle()) {
        neighbors.push_back(&net[x - 1][y + 1]);
    }
    if (x < getROWS() - 1 && y > 0 && !net[x + 1][y - 1].getVisited() && !net[x + 1][y - 1].getDeleted() &&
        !net[x + 1][y - 1].getObstacle()) {
        neighbors.push_back(&net[x + 1][y - 1]);
    }
    if (x < getROWS() - 1 && y < getCOLS() - 1 && !net[x + 1][y + 1].getVisited() &&
        !net[x + 1][y + 1].getDeleted() && !net[x + 1][y + 1].getObstacle()) {
        neighbors.push_back(&net[x + 1][y + 1]);
    }
